!(function($) {
  ...
  $('form.our-form').submit(function(e) {
    e.preventDefault();
  ...
  function form_submit(this_form, action, data) {
    $.ajax({
      ...
      if (message.msg == 'OK') {
        ...
        this_form.find('.loading').slideUp();
        if(!message.msg) {
          msg_text = 'Form submission failed and no error message returned from: ' + action + '<br>';
        }
        this_form.find('.error-message').slideDown().html(msg_text);
      }
    }).fail( function(data){
      console.log(data);
